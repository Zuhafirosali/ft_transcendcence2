...
class TournamentView(APIView):
	...
	def post(self, request):
		...
		if action != "create" and not tournament_id:
			return Response({"statusCode": 400, "message": "Tournament ID is required"}, status=status.HTTP_400_BAD_REQUEST)

		if tournament_id and not str(tournament_id).isdigit():
			return Response({"statusCode": 400, "message": "Invalid tournament ID"}, status=status.HTTP_400_BAD_REQUEST)

		try:
			tournament = Tournament.objects.get(id=tournament_id)
		except Tournament.DoesNotExist:
			return Response({"statusCode": 400, "message": "Tournament does not exist"}, status=status.HTTP_400_BAD_REQUEST)

		if action == 'join' and not alias:
			return Response({"statusCode": 400, "message": "Alias name is required to join a tournament"}, status=status.HTTP_400_BAD_REQUEST)
	...
	def join_tournament(self, tournament, player, alias):
		# Check if tournament is still open for new players
		MAX_PLAYERS = 4
		current_player_count = PlayerTournament.objects.filter(tournament=tournament).count()
	
		if current_player_count >= MAX_PLAYERS:
			return Response({"statusCode": 400, "message": "Tournament is full"}, status=status.HTTP_400_BAD_REQUEST)

		# Check if player is already in another tournament
		if Tournament.objects.filter(playertournament__player_id=player.id, status=StatusChoices.PENDING.value).exists():
			return Response({"statusCode": 400, "message": "Player already in a tournament"}, status=status.HTTP_400_BAD_REQUEST)

		# Ensure alias name is unique within the same tournament
		if PlayerTournament.objects.filter(tournament=tournament, player__alias_name=alias).exists():
			return Response({"statusCode": 400, "message": "Alias name is already taken in this tournament"}, status=status.HTTP_400_BAD_REQUEST)

		# Assign alias only if everything is successful
		player.alias_name = alias
		player.save()
