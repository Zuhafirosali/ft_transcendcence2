...
class ProfilViewList(
	...
	def get_queryset(self):
			username = self.kwargs.get('username', None)  # We get the username from the URL
...
def serve_dynamic_image(request, filename):
    ...
    if os.path.exists(image_path):
        with open(image_path, 'rb') as f:
            return HttpResponse(f.read(), content_type="image/jpeg")  # Or any other type that is suitable
    else:
