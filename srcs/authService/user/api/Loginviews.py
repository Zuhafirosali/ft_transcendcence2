    ...
    def post(self, request, *args, **kwargs):
        ...
        if user_check is not None:
            ...
            if userProfil.two_factory == True and code_2fa == "":
                return Response({"2fa required"}, status=status.HTTP_202_ACCEPTED)
                ...
                if  check_code == True:
                    ...
                    return Response({"2fa code is wrong"}, status=status.HTTP_401_UNAUTHORIZED)
            else:
                return super().post(request, *args, **kwargs)
        else:
            return Response({"Login Incorrect, Check Your Password or username."}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if not refresh_token:
                ...
                return Response({"Unauthororized"}, status=status.HTTP_401_UNAUTHORIZED)
            else:
                return Response({'Right'}, status=status.HTTP_200_OK)
        except Exception as e:
