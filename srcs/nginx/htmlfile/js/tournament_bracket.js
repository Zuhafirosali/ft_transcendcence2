...
async function initWebSocket_tournament() {
    if (ws_tournament && ws_tournament.readyState === WebSocket.OPEN) {
        console.log('Tournament WebSocket is already open');
        return;
    }else{
        token = getCookie('access_token')
        if(!token) {
            console.log('The user is not logged in, the WebSocket connection is not created.');
            return;
        }
        else{
            ws_tournament = new WebSocket(`${window.location.origin.replace(/^http/, 'ws')}/ws-tournament/${getCodeURL('tournament')}/?token=${getCookie('access_token')}`);
