...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        if self.scope['user'].is_authenticated:
            ...
            self.TeamGlobal = await self.get_TeamGlobal()
            await self.accept()
    ...
    async def receive(self, text_data):
        ...
        if m_type == 'chat_message':
            ...
            await self.send(text_data=json.dumps({
                'type': 'chat',
                'sender': self.TeamGlobal.username,
                'message': message,
                'chat_room': m_room,
                'photo': await self.get_profile_photo(self.TeamGlobal.username)
            }))
    ...
    async def chat_message(self, event):
        ...
        if await self.check_user_in_room(self.user, room):
                    ...
                    if self.user.username == sender:
                        ...
                        await self.send(
                                text_data=json.dumps({
                                    "type": "chat",
                                    "sender": self.TeamGlobal.username, 
                                    "message": 'YOU CANNOT SEND A MESSAGE TO SOMEONE YOU HAVE BLOCKED/UNBLOCKED!', 
                                    "photo": await self.get_profile_photo(self.TeamGlobal.username),
                                    'chat_room': room.roomName
                                })
                            )
            ...
            #If it is not in global chat, I need to check inside the room. If I am going to send an error message.
    ...
    @database_sync_to_async
    def get_TeamGlobal(self):
        TeamGlobal = User.objects.get(username='TeamGlobal')
        return TeamGlobal
